...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFD] text-[#1D1D1F] font-sans selection:bg-blue-100">
        ...
        {view === "upload" ? (
          <motion.div key="upload" className="max-w-4xl mx-auto pt-32 px-6 text-center space-y-12">
            ...
            <div className="relative group max-w-2xl mx-auto">
              ...
              >
                {isScanning ? <Activity className="animate-spin" size={18} /> : "Start Audit"}
              </button>
            </div>
          ...
          <motion.div key="results" className="max-w-7xl mx-auto p-8 space-y-12">
            ...
            {/* Anatomy Tab */}
            {activeTab === "anatomy" && scanResults && (
              <div className="grid grid-cols-3 gap-6">
                {scanResults.agents_reports.architect.components.map((comp: any, i: number) => (
            ...
            {/* Security Tab */}
            {activeTab === "security" && scanResults && (
              <div className="space-y-8">
                {scanResults.agents_reports.security.vulnerabilities.map((v: any, i: number) => (
                  <div key={i} className="bg-white p-8 rounded-[32px] border border-gray-100">
                    <div className="flex justify-between items-start mb-4">
                      <h4 className="text-xl font-bold">{v.pkg}</h4>
                      <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${v.severity === "Critical" || v.severity === "High" ? "bg-red-100 text-red-600" : "bg-yellow-100 text-yellow-700"}`}>
                        {v.severity}
                      </span>
                    </div>
                    <p className="text-gray-500 mb-2">{v.desc}</p>
                    <code className="text-sm font-mono block mb-2">$ {v.fix_command}</code>
                    <button
                      onClick={() => handleCopy(v.fix_command, `v-fix-${i}`)}
                      className={`px-4 py-2 rounded-xl font-bold text-xs transition-all flex items-center gap-2 ${copiedId === `v-fix-${i}` ? "bg-green-500 text-white" : "bg-gray-100 text-black hover:bg-gray-200"}`}
                    >
                      {copiedId === `v-fix-${i}` ? <><Check size={14} /> Applied</> : <><Copy size={14} /> Copy Fix Command</>}
                    </button>
                  </div>
                ))}
              </div>
      ...
      {isScanning && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 bg-white/90 backdrop-blur-2xl z-[100] flex items-center justify-center p-8">
